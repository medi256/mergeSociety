    ...
    <div className="lesson-wrapper">
      ...
      <div className="lesson-container">
        ...
        <p>
          ...
          statement helps us handle errors gracefully. Now, let’s dive deeper
          into how we can <strong>throw</strong> and <strong>handle</strong> our
          own errors in JavaScript. Throwing errors manually gives us more
          control over how and when an error should occur.
        </p>

        <h2>What Does "Throwing an Error" Mean?</h2>
        <p>
          When we say we are <strong>throwing an error</strong>, it means we are
          intentionally creating an error when something unexpected happens in
          our code. For example, if a user enters an invalid value or a
          condition we expect isn't met, we can throw an error to alert the
...
try {
  ...
  console.log('Error:', error.message); // Handling the error
        ...
        <p>
          In this example, the <code>checkAge</code> function checks if the age
          is a negative number. If it is, we <strong>throw</strong> an error
          using <code>throw new Error()</code> with a custom message: "Age
          cannot be negative". When this happens, the code inside the{" "}
          <code>try</code> block will stop running, and the control moves to the{" "}
          <code>catch</code> block, where the error is handled.
        </p>
        ...
        <ul>
          <li>
            <strong>Input Validation:</strong> If a user enters invalid data
            (like a negative age), you can throw an error to prevent further
            processing.
          </li>
          <li>
            <strong>Unexpected Conditions:</strong> If a condition that should
            never happen does occur (for example, dividing by zero), you can
            throw an error to stop the code from continuing in an unpredictable
            state.
          </li>
          <li>
            <strong>Debugging:</strong> Throwing custom errors with clear
            messages helps you track down problems in your code more easily.
          </li>
        </ul>

        <p>
          By throwing errors where something goes wrong, you ensure that your
          program doesn't continue running with incorrect data or in an
          unexpected state. Instead, you can handle the error and take the
          appropriate action, such as showing an error message to the user or
...
try {
  ...
  console.log('Error:', error.message); // Handling the error
        ...
        <p>
          ...
          email contains an "@" symbol. If not, it throws a custom error:
          "Invalid email format". The <code>catch</code> block then handles that
          error by printing the error message.
        </p>

        <h2>How to Handle Errors</h2>
        <p>
          Once an error is thrown, we need to handle it in the{" "}
          <strong>Catch</strong> block. Handling errors means deciding what to
          do when something goes wrong. For example:
        </p>
        ...
        <ul>
          ...
          <li>
            ...
            technical error, you can display a message like "Please enter a
            valid email."
          </li>
          <li>
  ...
  console.log('Please enter a valid email address.');
        ...
        <ul>
          <li>
            ...
            likely to occur (like user input or network requests), but avoid
            using it for every single line of code.
          </li>
          <li>
            <strong>Always Provide Helpful Error Messages:</strong> When
            throwing custom errors, make sure the messages are clear and
            describe what went wrong. This will make debugging much easier.
          </li>
          <li>
            <strong>Don’t Hide Errors:</strong> Handle errors in a way that
            allows you to understand and fix them. Don’t just suppress the error
            message without fixing the underlying issue.
          </li>
        </ul>
        ...
        <p>
          ...
          appropriate action to handle it. Whether you are validating user
          input, making network requests, or debugging your code, throwing and
          catching errors is a powerful tool to make your JavaScript programs
          more reliable.
        </p>
